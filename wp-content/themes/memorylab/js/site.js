  ...
  if (submitBtn) {
    submitBtn.addEventListener("click", function (e) {
      ...
      if (isValid) {
        ...
        const results = document.getElementById("show-ai-results");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initRow(row, direction) {
    ...
    waitForImages(row).then(() => {
      ...
      if (singleWidth <= 0) return; // защита от деления/бесконечного цикла
  ...
  if (row1) initRow(row1, 'right');
  if (row2) initRow(row2, 'left');
});
/* 
-------------------
...
(function() {
  ...
  if (!modalOverlay) return; // если попапа нет на странице — выходим
...
(function initHeroGradient() {
  ...
  try {
    ...
    console.error('NeatGradient init error:', e);
  }
})();
...
/* ==============
   ...
   ==============*/
document.addEventListener('DOMContentLoaded', function () {
  // Проверяем, что Swiper загружен И что нужные контейнеры есть на странице
  const mainEl = document.querySelector('.swiper-main');
  const thumbsEl = document.querySelector('.swiper-thumbs-slider');
  ...
  // Если слайдера нет на странице — просто выходим, НЕ ломая остальной JS
  if (!mainEl || !thumbsEl) {
    console.warn('Swiper: контейнеры .swiper-main или .swiper-thumbs-slider не найдены — инициализация пропущена.');
    return;
  }
  ...
  try {
    ...
    console.error('Ошибка инициализации mainSwiper:', e);
    return;
  }
  ...
  function initThumbsSwiper() {
    ...
    try {
      ...
      console.error('Ошибка инициализации thumbsSwiper:', e);
      return;
    }
  ...
  (function initGalleryModal() {
    ...
    if (!overlay || !closeBtn || !content || !swiperWrapper) return;
    ...
    function openModal(startIndex = 0) {
      ...
      try {
        ...
        console.error('Ошибка инициализации gallery modal Swiper:', e);
      }
    }
