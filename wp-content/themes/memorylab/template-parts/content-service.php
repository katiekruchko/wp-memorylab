<?php
/**
 * For single-staff page, section service (райдер)
 *
...
$rider_items = array_filter((array)$pod->field('rider-text'), function($item) {
...
if (empty($rider_items)) {
    return;
}
?>

<section class="service-wrap gap-section">
              <h2 class="h2-catalog">
                <span class="text-gradient">Технический</span> райдер
              </h2>
    
    <div class="wrap-tiles">
        <?php foreach (array_values($rider_items) as $index => $item): ?>
            <?php 
